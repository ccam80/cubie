...
try:  # local root
    Pig7TVE = np.genfromtxt(
        "./pig_7_TVE_normalised.csv", delimiter=",", dtype=precision
    )[:, np.newaxis]
    ...
    valid_indices = np.genfromtxt(
        "./nic_numerical_comparison/valid_indices.csv",
        delimiter=",",
        dtype=int,
    )
except:
    try:  # repo root
        Pig7TVE = np.genfromtxt(
            "./src/scratch/pig_7_TVE_normalised.csv",
            delimiter=",",
            dtype=precision,
        )[:, np.newaxis]
        matlab_results = np.genfromtxt(
            "./src/scratch/nic_numerical_comparison/comparison_waveforms.csv",
            delimiter=",",
            dtype=precision,
        )
        input_values = np.genfromtxt(
        ...
        try:  # src root
            Pig7TVE = np.genfromtxt(
                "./scratch/pig_7_TVE_normalised.csv",
                delimiter=",",
                dtype=precision,
            )[:, np.newaxis]
            matlab_results = np.genfromtxt(
                "./scratch/nic_numerical_comparison/comparison_waveforms.csv",
                delimiter=",",
                dtype=precision,
            )
            input_values = np.genfromtxt(
