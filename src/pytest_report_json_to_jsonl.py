...
def json_to_jsonl(input_file_path, output_file_path):
    ...
    try:
        ...
        if (
            ...
            print(
                f"Info: extracting {len(data['tests'])} test records from report"
            )
            data = data["tests"]

        if not isinstance(data, list):
            print(
                "Warning: Input JSON is not a list of objects. Each object will be written as a line."
            )
        ...
        print(
            f"Error: Could not decode JSON from '{input_file_path}'. Check file format."
        )
