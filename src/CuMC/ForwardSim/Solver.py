...
class Solver:
    ...
    def active_output_arrays(self) -> ActiveOutputs:
        """Exposes
        :attr:`~CuMC.ForwardSim.BatchSolverKernel.active_output_arrays` from
        the child BatchSolverKernel object."""
        return self.kernel.active_output_arrays

    @property
    def state_dev_array(self) -> 'MappedNDArray':
        """Exposes :attr:`~CuMC.ForwardSim.BatchSolverKernel.state_dev_array
    ...
    def save_time(self) -> bool:
        """Exposes :attr:`~CuMC.ForwardSim.BatchSolverKernel.save_time` from
        the child BatchSolverKernel object."""
        return self.kernel.save_time

    @property
    def output_types(self) -> list[str]:
        """Exposes :attr:`~CuMC.ForwardSim.BatchSolverKernel.output_types`
        from the child BatchSolverKernel object."""
        return self.kernel.output_types

    @property
    def input_variables(self) -> List[str]:
        """Exposes :attr:`~CuMC.ForwardSim.BatchSolverKernel.input_variables
        ` from the child BatchSolverKernel object."""
