...
| `cubie_cache.py` | File-based persistence of compiled kernels: `CUBIECache*`, `CachePolicy`, `CubieCacheHandler`, `ALL_CACHE_PARAMETERS`. Built on the backend's cache bases from `cuda_simsafe` (numba-cuda `_Kernel` serialization or the MLIR compile-result scheme). |
