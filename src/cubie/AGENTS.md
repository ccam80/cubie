...
| `cuda_backend.py` | Resolves which CUDA frontend cubie compiles against: `CUDA_BACKEND` (`"numba-cuda"` or `"mlir"`) and `IS_MLIR`, from the installed package (exactly one frontend may be installed; `CUBIE_CUDA_BACKEND` picks explicitly when both are). Consumed by `cuda_simsafe`, `cubie_cache`, and `__init__` (which imports `_numba_cuda_compat` or `_mlir_compat` accordingly). |
...
- **Import every CUDA symbol from `cuda_simsafe`** — the `cuda` module object
  itself (`from cubie.cuda_simsafe import cuda`), scalar types (`int32`, …),
  `numba_from_dtype`, the warp intrinsics (`selp`, `activemask`, `all_sync`,
  `any_sync`, `syncwarp`), the store write-through hint `stwt`, `from_dtype`,
  `is_devfunc`/`is_cuda_array`, `INLINE_ALWAYS`, and the memory-manager/array
  stand-ins. Never import a CUDA frontend package (`numba.cuda`,
  `numba_cuda_mlir`) directly: the hub maps the surface onto whichever backend
  is installed and keeps CUDASIM working on numba-cuda. **Never set
