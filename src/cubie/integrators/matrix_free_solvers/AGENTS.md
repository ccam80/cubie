...
- `preconditioner` (optional; `None` → search direction is `rhs`); one wide
  signature for every preconditioner, single or chained:
  `(state, parameters, drivers, base_state, t, h, a_ij, rhs,
  preconditioned_vec, jvp, scratch, chain_scratch)` (cached variant inserts
  `cached_aux` after `drivers`). Single preconditioners ignore
  `chain_scratch`; only a chained composition writes it.
