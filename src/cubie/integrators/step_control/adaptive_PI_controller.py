...
class AdaptivePIController(BaseAdaptiveStepController):
    ...
    def __init__(self,
        ...
        config = PIStepControlConfig(precision=precision,
                                     dt_min=dt_min,
                                     dt_max=dt_max,
                                     atol=atol,
                                     rtol=rtol,
    ...
    def build(self):
        ...
        def controller_PI(
            ...
            dt[0] = dt_new (clamped)
            accept_out[0] = accept (as Int32: 1 for True, 0 for False)
            scaled_error[0] = scaled error
            error_integral_array[0] = running error integral

            Returns retcode as int32.
            """
            gain_old = local_temp[0:1]
            for i in range(n):
