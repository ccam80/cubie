...
| `stage_predictors.py` | `DenseStagePredictor(CUDAFactory)`: in-place read-ahead of a persistent stage-increment vector over the runtime step-size ratio; tableau-derived, no codegen. FIRK and DIRK steps own one as a buffer-registry child. |
