...
import sympy as sp
from cubie.systemmodels.symbolic.dxdt import DXDT_TEMPLATE, \
    generate_dxdt_fac_code
from cubie.systemmodels.symbolic.jacobian import JVP_TEMPLATE, VJP_TEMPLATE, \
    generate_jvp_code
from cubie.systemmodels.symbolic.numba_cuda_printer import CUDAPrinter, \
    print_cuda_multiple
...
class ODEFile:
    ...
    def generate_dxdt_fac(self,
        ...
        if not self._dxdt_generated:
            ...
            code = generate_dxdt_fac_code(equations,index_map, func_name)
            self.add_function(code, func_name)

    def get_dxdt_fac(self,
                     equations: Iterable[Tuple[sp.Symbol, sp.Expr]],
                     index_map: IndexedBases):
        self.generate_dxdt_fac(equations, index_map)
    ...
    def generate_jvp_fac(self,
        ...
        if not self._jvp_generated:
            ...
            code = generate_jvp_code(equations, index_map, func_name)
            self.add_function(code, func_name)

    def get_jvp_fac(self,
                     equations: Iterable[Tuple[sp.Symbol, sp.Expr]],
                     index_map: IndexedBases):
        self.generate_jvp_fac(equations, index_map)
    ...
    def get_vjp_fac(
        ...
        self.generate_vjp_fac(equations, index_map)
        return self._import_function("jvp_factory")
    ...
    def get_factory(self, func_type, equations, index_map):
        if func_type == "dxdt":
            return self.get_dxdt_fac(equations, index_map)
        elif func_type == "jvp":
            return self.get_jvp_fac(equations, index_map)
        elif func_type == "vjp":
            return self.get_vjp_fac(equations, index_map)
        else:
