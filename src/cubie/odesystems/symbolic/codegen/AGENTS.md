...
| `jacobian.py` | Pure-symbolic (emits no CUDA): `generate_jacobian` (full analytic Jacobian via chain rule over auxiliary assignments) and `generate_analytical_jvp` (returns `JVPEquations` with `j_ij` entry symbols and `jvp[i]` product terms). Memoised in a module-level `_cache` keyed by `get_cache_key`. The module docstring attributes the approach to `chaste_codegen._jacobian` (MIT). |
