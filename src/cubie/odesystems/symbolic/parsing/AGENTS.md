...
| `function_inspector.py` | AST analysis of a callable ODE. `inspect_ode_function` → `FunctionInspection`; `_OdeAstVisitor` collects state/constant accesses, assignments, calls, unrolls `for`, synthesises `IfExp` from if/elif/else, rejects unsupported constructs; `AstToSympyConverter` maps AST nodes to SymPy. |
| `function_parser.py` | `parse_function_input` — bridges `FunctionInspection` to the parser's `(equation_map, funcs, new_params)` triple: builds the symbol map, emits auxiliary/observable/dxdt equations, inlines `dx = expr; return [dx]` aliases. |
