...
| `cellml.py` | `load_cellml_model` — wraps optional `cellmlmanip`, sanitises symbol names, substitutes the model's Dummy atoms in SymPy (one `xreplace` pass per equation, whose rebuild re-evaluation canonicalises numeric coefficients and constant exponential offsets so kernels share `exp(k*V)` factors), converts to engine IR with a shared memo, splits differential vs algebraic equations, classifies constants/parameters/observables, then calls `parse_input` with IR pairs. Cache-aware (early + post-GUI checks). |
