...
class ODEData(CUDAFactoryConfig):
    ...
    solver_preconditioner_order
        Polynomial order of generated Neumann preconditioners.
        Defaults to ``2``.
    solver_tableau_digest
        Digest of the FIRK tableau baked into generated multi-stage
        helpers. Defaults to ``""``.

    Notes
    -----
    The ``solver_*`` fields are compile-critical inputs to the solver
    helpers generated by
    :meth:`~cubie.odesystems.symbolic.symbolicODE.SymbolicODE.get_solver_helper`,
    which is their single writer. Their names carry the ``solver_``
    prefix so that flat update dictionaries routed through
    :meth:`~cubie.odesystems.baseODE.BaseODE.update` cannot confuse
    them with system constants named ``beta`` or ``gamma``.
    """
    ...
    solver_preconditioner_order: int = field(default=2, converter=int)
    solver_tableau_digest: str = field(
