...
This module adapts CuPy synchronous and asynchronous memory pools for
Numba's External Memory Manager interface. It also exposes helpers that
align CuPy's current stream selection with streams generated by
:mod:`numba.cuda` so that stream-ordered allocations behave
...
:class:`CuPyAsyncNumbaManager`
    Numba EMM plugin using CuPy's asynchronous memory pool.

:class:`CuPySyncNumbaManager`
    Numba EMM plugin using CuPy's synchronous memory pool.

See Also
--------
:class:`~cubie.memory.mem_manager.MemoryManager`
    Coordinates allocation through these plugins.
:mod:`~cubie.cuda_simsafe`
    Provides the base memory manager classes and CUDASIM shims.
"""
...
class CuPyNumbaManager(GetIpcHandleMixin, HostOnlyCUDAMemoryManager):
    ...
    def reset(self, stream: Optional[Any] = None) -> None:
        ...
        ----------
        stream
            CuPy stream used for asynchronous deallocation. When
            omitted, the pool frees blocks synchronously.
...
class CuPyAsyncNumbaManager(CuPyNumbaManager):
    ...
    Notes
    -----
    Uses CuPy's asynchronous memory pool which provides
