...
def random_system_values(coeffs, precision=np.float64, randscale=1e6):
    ...
    drivers = generate_test_array(
        precision, 1, style="random", scale=randscale
    )
...
class TestDecays(SystemTester):
    ...
    def instantiate_system(
        self, system_class, coefficients, precision=np.float32
    ):
        """Instantiate the decays system with default parameters."""
        self.system_instance = system_class(
            precision=precision, coefficients=coefficients
        )
    ...
    def test_constants_edit(
        ...
        for (
            ...
            self.system_instance.compile_settings.constants.values_dict.items()
        ):
        ...
        with pytest.raises(AssertionError):
            assert_allclose(
                ...
                expected_dx,
                rtol=rtol,
                err_msg="pre-rebuild mismatch - expected",
            )
