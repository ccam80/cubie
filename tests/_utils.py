...
def calculate_expected_summaries(state,
    ...
    - output_types: List of output function names to apply (e.g. ["mean", "preaks[3]", "max", "rms"])
    ...
    cleaned_output_list, n_peaks_dict = process_outputs_list(output_types)
...
def calculate_single_summary_array(input_array,
    ...
    - summary_size_per_state: Number of summary values per state (e.g. 1 for mean, 1 + n_peaks for mean and preaks[n])
    - output_functions_list: List of output function names to apply (e.g. ["mean", "preaks[3]", "max", "rms"])
    - n_peaks: Number of peaks to find in the "preaks[n]" output function
