...
from tests._utils import StepResult, _run_device_step
...
@pytest.mark.parametrize(
    "solver_settings_override", _CTRL_OVERRIDES, indirect=True,
)
@pytest.mark.parametrize(
    "step_setup",
    [
        ...
        pytest.param(
            {"dt0": 0.005, "error": np.full(3, 5e-3),
             "local_mem": np.array([0.005, 0.8])},
            id="high_err_with_mem",
        ),
    ],
    indirect=True,
)
def test_single_step_matches_cpu(
    step_controller,
    ...
    """Device controller dt and local memory match CPU reference."""
...
def test_cpu_gpu_sequence_agree(
    ...
    """dt updates and acceptance match CPU across a 5-step sequence."""
...
@pytest.mark.parametrize(
    "solver_settings_override", _EQUIV_OVERRIDES, indirect=True,
)
def test_sequential_acceptance_matches(
    ...
    """dt updates and acceptance match through rejections (6 steps)."""
...
def test_rejection_retains_previous_state(
    ...
    """Both controllers agree on rejection bookkeeping."""
