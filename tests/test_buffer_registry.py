...
class TestMultipleFactories:
    ...
    def test_clear_one_factory_doesnt_affect_others(self):
        ...
        self.registry.clear_factory(factory1)

        assert factory1 not in self.registry._contexts
        assert factory2 in self.registry._contexts
        assert self.registry.shared_buffer_size(factory2) == 50
...
class TestCrossTypeAliasing:
    ...
    def test_shared_alias_with_local_location_raises(self):
        self.registry.register('parent', self.factory, 100, 'shared')
        with pytest.raises(ValueError, match="cannot alias shared buffer"):
            self.registry.register(
                'child', self.factory, 30, 'local', aliases='parent'
            )

    def test_persistent_alias_of_nonpersistent_local_raises(self):
        self.registry.register('parent', self.factory, 100, 'local')
        with pytest.raises(ValueError, match="cannot alias.*non-persistent"):
            self.registry.register(
