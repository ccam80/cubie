...
class TestGenericLoopAlgorithm(LoopAlgorithmTester):
    ...
    def expected_answer(
        ...
        if save_state:
            ...
            expected_state_output = np.zeros(
                (n_samples, n_saved_states + 1 * save_time), dtype=precision
            )
            expected_state_output[:, :n_saved_states] = inputs[
                "initial_values"
            ][saved_state_indices][np.newaxis, :]
            if save_time:
                expected_state_output[:, -1] = np.arange(
                    n_samples, dtype=precision
                )
        else:
        ...
        if save_observables:
            ...
            expected_observables = np.zeros(
                (n_samples, n_saved_observables), dtype=precision
            )
            expected_observables[:, :] = inputs["initial_values"][
                np.newaxis, (saved_observable_indices % n_states_total)
            ]
        else:
