    ...
    if run_settings_override:
        ...
        for key, value in run_settings_override.items():
            if hasattr(defaults, key):
                ...
                warn(
                    f"Unknown run setting '{key}' provided; ignoring.",
                    UserWarning,
                )
...
def inputs_dict(system, precision, **kwargs):
    ...
    if system.sizes.drivers > 0:
        drivers[:, 0::25] = system.precision(
            1.0
        )  # Set every 25th sample to 1.0
    ...
    if "initial_values" in kwargs:
        ...
        if isinstance(initial_values_edits, np.ndarray):
            ...
            assert initial_values_edits.ndim == 1, (
                "Initial values must be a 1D array."
            )
            ...
            if provided_length < required_length:
                ...
                if provided_length > required_length:
                    warn(
                        "Redundant initial values provided; extra values are discarded.",
                        UserWarning,
                    )
    ...
    if "parameters" in kwargs:
        ...
        if isinstance(parameters_edits, np.ndarray):
            ...
            if provided_length < required_length:
                ...
                if provided_length > required_length:
                    warn(
                        "Redundant parameters provided; extra values are discarded.",
                        UserWarning,
                    )
    ...
    if "forcing_vectors" in kwargs:
        ...
        if isinstance(forcing_override, np.ndarray):
            assert forcing_override.shape[0] == system.sizes.drivers, (
                "forcing_vectors override must have system.num_drivers rows."
            )
