...
class TestSolveResultStaticMethods:
    ...
    def test_combine_time_domain_arrays(self, state_flag, obs_flag, expected):
        ...
        s = np.array([[[1, 2]], [[3, 4]]])
        o = np.array([[[5, 6]], [[7, 8]]])
        result = SolveResult.combine_time_domain_arrays(s, o, state_flag, obs_flag)
    ...
    def test_combine_summaries_array(self, state_flag, obs_flag, expected):
        ...
        result = SolveResult.combine_summaries_array(s, o, state_flag, obs_flag)
        expected_result = expected(s, o)
        assert np.array_equal(result, expected_result)
    ...
    def test_cleave_time_with_time_saved(self):
        ...
        time_result, state_result = SolveResult.cleave_time(state, time_saved=True)
    ...
    def test_cleave_time_without_time_saved(self):
        ...
        time_result, state_result = SolveResult.cleave_time(state, time_saved=False)
...
class TestSolveResultInstantiation:
    ...
    def test_instantiation_type_equivalence(self, solver_with_arrays):
        ...
        full_result = SolveResult.from_solver(solver_with_arrays, results_type='full')

        # Create other types directly
        numpy_result = SolveResult.from_solver(solver_with_arrays, results_type='numpy')
        ...
        for key in numpy_result.keys():
            if isinstance(numpy_result[key], np.ndarray):
                assert np.array_equal(numpy_result[key], full_result.as_numpy[key])
            else:
        ...
        for key in numpy_per_summary_result.keys():
            if isinstance(numpy_per_summary_result[key], np.ndarray):
                assert np.array_equal(
                    numpy_per_summary_result[key], full_result.as_numpy_per_summary[key]
                )
            else:
                assert numpy_per_summary_result[key] == full_result.as_numpy_per_summary[key]
        assert pandas_result["time_domain"].equals(full_result.as_pandas["time_domain"])
        assert pandas_result["summaries"].equals(full_result.as_pandas["summaries"])
    ...
    def test_from_solver_full_instantiation(self, solver_with_arrays):
        ...
        assert result._stride_order == solver_with_arrays.kernel.output_arrays.host._stride_order
    ...
    def test_from_solver_numpy_instantiation(self, solver_with_arrays):
        ...
        result = SolveResult.from_solver(solver_with_arrays, results_type='numpy')
    ...
    @pytest.mark.parametrize("solver_settings_override",
                            [{"output_types": ["state", "observables", "time", "mean", "rms"]}],
                            indirect=True)
    def test_from_solver_numpy_per_summary_instantiation(self, solver_with_arrays):
        """Test numpy per summary dict instantiation from solver."""
        result = SolveResult.from_solver(solver_with_arrays, results_type='numpy_per_summary')

        assert isinstance(result, dict)
        assert 'mean' in result
        assert 'time_domain_array' in result
        # Check that summary types are separated
        for summary_type in solver_with_arrays.summary_legend_per_variable.values():
    ...
    def test_from_solver_pandas_instantiation(self, solver_with_arrays):
        ...
        result = SolveResult.from_solver(solver_with_arrays, results_type='pandas')
...
class TestSolveResultFromSolver:
    ...
    def test_time_domain_legend_from_solver(self, solver_with_arrays):
        ...
        assert any(v.startswith('o') for v in legend.values() if isinstance(v, str))
    ...
    def test_summary_legend_from_solver(self, solver_with_arrays):
        ...
        summary_metrics = [s for s in solver_with_arrays.output_types if
                          any(s.startswith(metric) for metric in ['mean', 'rms'])]
...
class TestSolveResultProperties:
    ...
    def test_as_numpy_property(self, solver_with_arrays):
        ...
        assert np.array_equal(numpy_dict['time_domain_array'], result.time_domain_array)
    ...
    def test_as_pandas_property(self, solver_with_arrays):
        ...
        if result.time_domain_array.ndim == 3:
            ...
            expected_runs = result.time_domain_array.shape[result._stride_order.index("run")]
...
class TestSolveResultDefaultBehavior:
    ...
    def test_custom_stride_order(self):
        ...
        # Test with mock data
        result.time_domain_array = np.random.rand(5, 4, 3)  # matches custom order
...
class TestSolveResultPandasIntegration:
    ...
    def test_pandas_shape_consistency(self, solver_with_arrays):
        ...
        result = SolveResult.from_solver(solver_with_arrays)
        td_df, sum_df = result.as_pandas["time_domain"], result.as_pandas["summaries"]
        ...
        if result.summaries_array.size > 0:
            expected_sum_cols = len(result.summaries_legend) * array_shape[1] if result.summaries_array.ndim == 3 else len(result.summaries_legend)
    ...
    def test_pandas_time_indexing(self, solver_with_arrays):
        ...
        if result.time is not None and result.time.size > 0:
            ...
            if result.time.ndim == 1:
                ...
                expected_time = result.time[:, 0] if result.time.shape[1] > 0 else result.time.flatten()
...
class TestSolveResultErrorHandling:
    ...
    def test_empty_summaries_per_summary_arrays(self):
        ...
        result = SolveResult()
        result._active_outputs = ActiveOutputs(state_summaries=False, observable_summaries=False)
