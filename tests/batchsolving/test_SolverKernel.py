...
@pytest.mark.parametrize("system_override, solver_settings_override, batch_settings_override",
                         (({},
                           {},
                           {}),
                         ("ThreeChamber",
...
def test_run(solverkernel, batch_input_arrays, solver_settings, square_drive, batch_settings,
    ...
    output_length = int(np.ceil(solver_settings['duration'] / solver_settings['dt_save']))
    summaries_length = int(np.ceil(solver_settings['duration'] / solver_settings['dt_summarise']))
    ...
    expected_state_summaries_shape = (summaries_length, numruns, len(solverkernel.summarised_state_indices) * output_summaries_height)
    ...
    for i, (expected_state_output, expected_observables_output) in enumerate(expected_batch_answers_euler):
        ...
        if active_output_arrays.state:
            assert_allclose(expected_state_output, state[:,i,:], atol=atol, rtol=rtol,
                            err_msg="Output does not match expected.")
        if active_output_arrays.observables:
            assert_allclose(expected_observables_output, observables[:,i,:], atol=atol, rtol=rtol,
                            err_msg="Observables do not match expected.")
        if active_output_arrays.state_summaries:
            assert_allclose(expected_state_summaries, state_summaries[:,i,:], atol=atol, rtol=rtol,
                            err_msg="Summary states do not match expected.")
        if active_output_arrays.observable_summaries:
            assert_allclose(expected_obs_summaries, observable_summaries[:,i,:], atol=atol, rtol=rtol,
...
@pytest.fixture(scope="function")
def expected_batch_answers_euler(system, solverkernel, batch_input_arrays, square_drive):
    inits, params = batch_input_arrays
    ...
    for i in range(inits.shape[0]):
        output_sets.append(cpu_euler_loop(system, inits[i,:], params[i,:], driver_vec, dt, output_dt,
...
def expected_batch_summaries(expected_batch_answers_euler, solverkernel,
    ...
    def test_expected_summaries(expected_summaries):
        ...
    """
    _, summarise_every, _ = solverkernel.single_integrator._integrator_instance.compile_settings.fixed_steps
    expected_summaries = []
    for (expected_state, expected_output) in expected_batch_answers_euler:
        expected_summaries.append(calculate_expected_summaries(
                expected_state,
                expected_output,
                summarise_every,
