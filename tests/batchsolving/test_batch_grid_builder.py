...
def test_combinatorial_and_verbatim_grid(system):
    ...
    request = {param.names[0]: np.array([0, 1]), param.names[1]: np.array([10, 20])}
...
def test_grid_size_errors(system):
    ...
    param = system.parameters
    with pytest.raises(ValueError):
        BGB.verbatim_grid({param.names[0]: [1, 2, 3],
...
def test_combine_grids(example_arrays):
    ...
    result_grid1, result_grid2 = BGB.combine_grids(grid1, grid2, kind='combinatorial')
    ...
    result_grid1_v, result_grid2_v = BGB.combine_grids(grid1_v, grid2_v, kind='verbatim')
...
def test_grid_arrays_1each(grid_builder, batch_request, batch_settings):
    ...
    initial_values_array, params_array = grid_builder.grid_arrays(batch_request, kind=kind)
    assert initial_values_array.shape[1] == grid_builder.states.values_array.shape[0]
    assert params_array.shape[1] == grid_builder.parameters.values_array.shape[0]
...
def test_grid_arrays_2and2(system, grid_builder, batch_settings):
    ...
    kind = batch_settings['kind']
    initial_values_array, params_array = grid_builder.grid_arrays(request, kind=kind)
...
@pytest.mark.parametrize("mixed_type, params_type,states_type",
                         list(itertools.product(['dict', 'seq', 'array', None],
                              repeat=3)))
def test_call_input_types(grid_builder, system,
    ...
    if (mixed_type, states_type, params_type) not in valid_combos:
        with pytest.raises(TypeError):
            initial, param = grid_builder(params=params,
                                                states=states, request=request)
        return
    else:
        initial, param = grid_builder(request=request, params=params,
                                            states=states)
...
def test_call_outputs(system, grid_builder):
    ...
    inits, params = grid_builder(params=testarray1, states=testarray1,
                                      kind='combinatorial')
    ...
    inits, params = grid_builder(params=testparamdict,
...
def test_docstring_examples(grid_builder):
    ...
    initial_states, parameters = grid_builder(params=params,
                                              states=states,
                                              kind='combinatorial')
    ...
    initial_states, parameters = grid_builder(params=params, states=states, kind='combinatorial')
    ...
    initial_states, parameters = grid_builder(request=request, kind='combinatorial')
    assert_array_equal(initial_states, expected_initial_large)
    assert_array_equal(parameters,expected_params_large)
