...
2. **Invoke agents from plan_new_feature or starting agent listed in prompt** with the issue content and
3. **Invoke subsequent agents in sequence, each with the context from the previous agent** until "return after" level is reached
...
4. Invoke taskmaster with detailed_implementer outputs
5. Invoke reviewer with taskmaster outputs
6. Invoke taskmaster with review outputs
7. Summarise and return to user
```
